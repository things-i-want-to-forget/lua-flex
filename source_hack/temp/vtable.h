...
public:
	VTable(void *object)
	{
		...
		while (GoodPtr(*last_index++));

		unsigned short size = offset(last_index, original_vt) / sizeof(vtindex);

		new_vt = new vtindex[size];
		while (last_index > original_vt)
		{
			new_vt[offset(last_index, original_vt) / sizeof(vtindex)] = *last_index;
			last_index--;
		}
